 ...
 */
    ...
    public function export_to_excel($trip_sheet_id)
    {
        $data_cndtnx = array('id' => $trip_sheet_id);
        $details = $this->admin_model->get_all_data('trip_sheet', $data_cndtnx, 'id', 'DESC');
        ...
        if ($details == false) {
            ...
            $data_cndtnx = array('trip_sheet_cargos.trip_sheet_id' => $trip_sheet_id);
            $trip_sheet_cargos = $this->admin_model->get_all_data_print('trip_sheet_cargos', $data_cndtnx, 'trip_sheet_cargos.sort_order', 'ASC');

            $spreadsheet = new Spreadsheet();
            $sheet = $spreadsheet->getActiveSheet();

            // Set the headers for the Excel file
            $sheet->setCellValue('A1', 'Trip NO');
            $sheet->setCellValue('B1', $details[0]->trip_sheet_name);
            $sheet->setCellValue('A2', 'Date');
            $sheet->setCellValue('B2', $details[0]->trip_date . ' ' . $details[0]->trip_time);
            $sheet->setCellValue('A3', 'Vehicle NO');
            $sheet->setCellValue('B3', $details[0]->vehicle_number);
            $sheet->setCellValue('A4', 'Driver');
            $sheet->setCellValue('B4', $details[0]->vehicle_drivername);
            $sheet->setCellValue('A5', 'Mobile');
            $sheet->setCellValue('B5', $details[0]->vehicle_drivermobilenumber);
            $sheet->setCellValue('A6', 'Helper');
            $sheet->setCellValue('B6', $details[0]->helper_name);
            $sheet->setCellValue('A7', 'Mobile');
            $sheet->setCellValue('B7', $details[0]->helper_mobilenumber);

            // Set the data for the cargo table
            $sheet->setCellValue('A10', 'S/N');
            $sheet->setCellValue('B10', '# No');
            $sheet->setCellValue('C10', 'Invoice No');
            $sheet->setCellValue('D10', 'Qnt');
            $sheet->setCellValue('E10', 'Weight');
            $sheet->setCellValue('F10', 'Place');
            $sheet->setCellValue('G10', 'State');
            $sheet->setCellValue('H10', 'Trip No');

            $rowCount = 11;
            foreach ($trip_sheet_cargos as $key => $cargo) {
                $sheet->setCellValue('A' . $rowCount, $key + 1);
                $sheet->setCellValue('B' . $rowCount, $cargo->grp_boxno);
                $sheet->setCellValue('C' . $rowCount, $cargo->goods_invoice);
                $sheet->setCellValue('D' . $rowCount, $cargo->tot_pcs);
                $sheet->setCellValue('E' . $rowCount, $cargo->tot_weight);
                $sheet->setCellValue('F' . $rowCount, $cargo->place);
                $sheet->setCellValue('G' . $rowCount, $cargo->state);
                $sheet->setCellValue('H' . $rowCount, $cargo->trip_no);
                $rowCount++;
            }

            // Set the filename and prompt the download
            $filename = 'TripSheet_' . $trip_sheet_id . '.xlsx';
