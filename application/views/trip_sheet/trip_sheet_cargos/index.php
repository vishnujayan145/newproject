...
<div class="main-content">
        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <!--<h4>Trip Number: <span id="trip_no_display"><?= $this->uri->segment(3) ?></span></h4>  -->
                            <h4>Trip Number: <span id="trip_no_display" class="highlighted-trip-number"><?= $this->uri->segment(3) ?></span></h4>
                            ...
                            <form action="<?= base_url() ?>trip_sheet/cargos/<?= $this->uri->segment(3) ?>" method="get" name="myxfrm" enctype="multipart/form-data" onsubmit="return validateForm();">
                                        ...
                                        <tbody id="sortable-table-body">
                                            ...
                                            if ($result != null) {
                                                ...
                                                foreach ($result as $key => $resultvlu) {
                                                    $total_cnt += 1;
                                                    $total_pcs += !empty($resultvlu->quantity) ? $resultvlu->quantity : 0;
                                                    $total_weight += !empty($resultvlu->weight) ? (int)$resultvlu->weight : 0;
                                            ?>
                                                    <tr id="row-<?= $key + 1 ?>">
                                                        <td align="center">
                                                            <input type="checkbox" name="bene_id[]" class="checkbox-item" value="<?= $resultvlu->invoice_number ?>" />
                                                        </td>
                                                        ...
                                                        <td><?= $resultvlu->invoice_number ?></td>
                                                        <td><?= $resultvlu->quantity ?></td>
                                                        ...
                                                        <td class="text-center">
                                                            <div class="btn-group" role="group" aria-label="Basic outlined example">
                                                                <button type="button" class="btn btn-outline-danger" onclick="location.href='<?= base_url() ?>trip_sheet/cargos_delete_process/<?= $this->uri->segment(3) ?>/<?= $resultvlu->id ?>'" data-toggle="tooltip" title="Delete cargo"><i class="mdi mdi-trash-can-outline" style="font-size: 22px"></i></button>
                                                                <button type="button" class="btn btn-outline-success" onclick="location.href='<?= base_url() ?>trip_sheet/cargos/<?= $this->uri->segment(3) ?>/<?= $resultvlu->id ?>'"> <i class="fas fa-pencil-alt" style="font-size: 22px"></i> </button>
                                                                ...
                                                                ?>
                                                                    ...
                                                                    <button type="button" title="Download POD" class="btn btn-outline-success" onclick="location.href='<?= base_url() ?>trip_sheet/download_pod/<?= $resultvlu->current_status_id ?>'"> <i class="fas fa-download" style="font-size: 22px"></i> </button>
                                                ...
                                                <tr style="font-weight:bold">
                                                    <td colspan="5"> Total Count :<?= $total_cnt ?> </td>
                                                    <td colspan="4">Total Pcs: <?= $total_pcs ?></td>
                                                    <td colspan="5">Total Weight : <?= $total_weight ?></td>
                                                </tr>
                                            <?php
                                            }
                                            ?>
                    ...
                    <div class=" mb-4">
                        ...
                        <div class="row justify-content-center">
                            <div class="col-xl-12">
                                ...
                                <form class="form-horizontal form-label-left ajaxModel" method="post" action="<?= base_url() ?>trip_sheet/cargo_change_status" enctype=multipart/form-data>
    ...
    <!-- end modal -->
    <script src="<?= base_url() ?>assets/libs/metismenu/Sortable.min.js"></script>

    <script>
        // Function to update the row numbers after dragging
        function updateRowNumbers() {
            document.querySelectorAll('#sortable-table-body .sl-no').forEach((cell, index) => {
                cell.textContent = index + 1;
            });
        }

        // Initialize Sortable on the table body for drag-and-drop
        new Sortable(document.getElementById('sortable-table-body'), {
            animation: 150,
            onEnd: function(evt) {
                updateRowNumbers(); // Update row numbers after each reorder
                console.log("Row moved from position:", evt.oldIndex, "to", evt.newIndex);
                // Optional: Send the new order to the server via AJAX if needed
            }
        });
    </script>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#inv_num').select2({
                ...
                ajax: {
                    url: '<?php echo base_url() ?>trip_sheet/get_invoices',
                    dataType: 'json',
                    delay: 250,
                    type: 'POST',
    ...
    <script>
        function showStatusModel(tripsheet_id, cargo_id_val, status, comment, invoice_no, tracking_no) {
            ...
            if (status == 0) {
                $("#asignModel").find("#commentDiv").css({
                    "display": "none"
                });
            }
            if (status == 1) {
        ...
        function showImageModel(imgUrl) {
            ...
            // $("#myImageModal").find("#down_pod").html(' <button onclick="download("https://api.indianlivecargo.com/storage/compress_1713508077.attachment.jpg")"> Download Image  </button>');          
        ...
        function showAllStatusModel() {
            ...
            if (status == 0) {
                $("#asignModelSelected").find("#sel_commentDiv").css({
                    "display": "none"
                });
            }
            if (status == 1) {
        ...
        function onStatusChangeMultiple(sel) {
            ...
            if (value == 0) {
                ...
                $("#asignModelSelected").find("#intransitDiv").css({
                    "display": "none"
                });
                $("#asignModelSelected").find("#fileDiv").css({
                    "display": "none"
                });
                $("#asignModelSelected").find("#deliveryDate").css({
                    "display": "none"
                });
        ...
        function onStatusChange(sel) {
            ...
            if (value == 0) {
                ...
                $("#asignModel").find("#intransitDiv").css({
                    "display": "none"
                });
                $("#asignModel").find("#fileDiv").css({
                    "display": "none"
                });
                $("#asignModel").find("#deliveryDate").css({
                    "display": "none"
                });
