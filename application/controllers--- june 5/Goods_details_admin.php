...
class Goods_details_admin extends CI_Controller {
	...
	{ 
		...
		if (isset($_GET['branch_id']) && $_GET['branch_id'] != null && $_GET['branch_id'] != 'all') {
			...
			$data_cndtn_branch=array('id' => $_GET['branch_id'] );
			$branch_details = $this->admin_model->getBranchName(  $_GET['branch_id'], $data_cndtn_branch);			 
			$this->data['branch_name'] =  $branch_details[0]->branch_name;
        }
	...
	public function print_pod()
	{
		if($this->uri->segment(3)==null){
			...
			if($details==false){
				...
				$this->load->view('goods_details_admin/print_pod');
