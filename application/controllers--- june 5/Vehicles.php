...
class Vehicles extends CI_Controller {
	...
	public function create()
	{
		...
		$this->load->view('head/header',$this->data);
		$this->load->view('vehicles/create');
		$this->load->view('head/footer');
	}
	...
	//check image selected or not
		...
		redirect(base_url().'vehicles/create');
	...
	public function update()
	{
		if($this->uri->segment(3)==null){
        	...
        	redirect(base_url().'vehicles');
				...
				$this->load->view('head/header',$this->data);
				$this->load->view('vehicles/update');
				$this->load->view('head/footer');
			}
		}
	}
	public function update_process()
	{
		...
		if (empty($_FILES['file_image']['name'])) {
        	...
        	redirect(base_url().'vehicles/update/'.$uiid);
		}
		else{
				// set_time_limit(0);
				// ini_set("upload_max_filesize",25);
				...
				// $datax=array(
				// 	'storename'=>$this->security->xss_clean($this->input->post('store_name')),
				// 	'ownername'=>$this->security->xss_clean($this->input->post('owner_name')),
				...
				// 	'contactnumber'=>$this->security->xss_clean($this->input->post('contactnumber')),
				// 	'alternatenumber'=>$this->security->xss_clean($this->input->post('alternatenumber')),
				// 	'idcardtype'=>$this->security->xss_clean($this->input->post('prooftype')),
				// 	'idproofimg'=>'assets/uploads/vehicles/'.$imgnme,
				// 	'admin_notes'=>$this->security->xss_clean($this->input->post('admin_notes')),
				// 	'updated_at'=>date('Y-m-d H:i:s'),
				// 	'status'=>$this->security->xss_clean($this->input->post('status'))
				// );
				// $lstid=$this->crud_model->edit_table_account('vehicles',$datax,$uiid,'id');
