			...
			foreach ($sheetData as $key => $val){
				...
				$lstid1 = $this->crud_model->create_table_account('import_data', $data_import); 
					...
					$datax=array(
						...
						'origin' =>        	isset($val['2']) ? trim($val['2']): "",
						'boxno' =>         	isset($val['3']) ? trim($val['3']): "",
						'tracking_no' =>   	isset($val['4']) ? trim($val['4']): "",
						'invoiceno' =>     	isset($val['5']) ? trim($val['5']): "",
						'pcs' =>           	isset($val['6']) ? trim($val['6']): "",
						'weight' =>        	isset($val['7']) ? trim($val['7']): "",
						'rewight' =>       	isset($val['8']) ? trim($val['8']): "",
						'received_pcs' =>  	isset($val['9']) ? trim($val['9']): "",
						'sender_address' => isset($val['10']) ? trim($val['10']): "",
						'reciever_address'=>isset($val['11']) ? trim($val['11']): "",
						'phone' =>         	isset($val['12']) ? trim($val['12']): "",
						'state' =>         	isset($val['13']) ? trim($val['13']): "",
						'district' =>      	isset($val['14']) ? trim($val['14']): "",
						'pincode' =>       	isset($val['15']) ? trim($val['15']): "",
						'goods_desc' =>    	isset($val['16']) ? trim($val['16']): "",
						'recieved_at_hub'=>	isset($val['17']) ? trim($val['17']): "",
						'received_date'=>  	isset($val['17']) ? date('d-m-Y', strtotime( trim($val['17']) ) ):"" ,
						'connecting_date'=>	isset($val['18']) ? trim($val['18']): "",
						'recieved_at_branch'=>isset($val['19']) ? trim($val['19']): "",
						'vendor'=>         	isset($val['20']) ? trim($val['20']): "",
						'docket'=>         	isset($val['21']) ? trim($val['21']): "",
						'url'=>            	isset($val['25']) ? trim($val['25']): "",
						'contact_no'=>     	isset($val['22']) ? trim($val['22']): "",
						'goods_status' =>   isset($val['23']) ? trim($val['23']): "",
						'remarks' =>       	isset($val['24']) ? trim($val['24']): "",
						'sendingdate' =>   	isset($val['26']) ? trim($val['26']): "",
						'recievingdate' =>  isset($val['27']) ? trim($val['27']): "",
						'trip_no' =>   $trip_no,
						'created_at' =>    date('Y-m-d H:i:s'),
